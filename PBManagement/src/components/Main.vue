  ...
  <div>
    <el-menu v-if="menuTree.length > 0" default-active="/" theme="dark" :router="true" :routerReplace="true" class="navbar navbar-fixed-top" mode="horizontal" @select="handleSelect">
      ...
      <template v-for="menu in menuTree">
        <el-menu-item v-if="menu.urlOpenType == 0" :index="menu.url">{{ menu.description }}</el-menu-item>
        <li v-else class="el-menu-item">
          <a class="external-links" :href="getUrl(menu.url)" target="_blank">{{ menu.description }}</a>
        </li>
      </template>
...
<script>
  ...
  export default {
    ...
    methods: {
      getUrl (url) {
        return url + '?uid=' + this.userInfo.userId + '&token=' + escape(this.userInfo.token);
      },
