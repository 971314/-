...
/**
 * Project: GmHall Comments: 管理系统会话时产生的缓存 JDK Version Used: JDK 1.8 Author: Wang
 * Wenchao Create Date: 2016/7/6 18:31 Version:
 * <版本号> ************************************* Modified By:
 * <修改人中文名或拼音缩写> Modified Date：<修改日期，格式:YYYY-MM-DD> Reason：<修改原因描述> Version：
 * <版本号>
 */
...
public class UserCacheManager extends CacheManager {
	...
	public boolean addSession(String account, String token) {
		if (!isExist(account, CacheKey.SESSIONID_SET)) {
			// 当前无用户登录，新增
			Map<String, Long> tokens = new HashMap<>();
			tokens.put(token, System.currentTimeMillis());
			add(account, CacheKey.SESSIONID_SET, tokens);
		} else {
			Map<String, Long> tokens = (Map<String, Long>) get(account, CacheKey.SESSIONID_SET);
			if (MAX_USER == 1) {
				tokens.clear();
				tokens.put(token, System.currentTimeMillis());
			} else {
	...
	/**
	 * 判断选择出该被踢出的客户, 按照时间顺序，踢出最早登录的 可以根据要求编写踢出策略，修改此方法即可
	 *
	 * @param account
	 *            客户ID
