        ...
        <div class="panel panel-default linkmanInfo allDivWidth">
            <div class="panel-body">
                <form class=" top17 formTop" role="form" data-validate="parsley">
                    <div class="form-group">
                        ...
                        <div class="btn-group">
                            <button type="button" class="btn btn-default dropdown-toggle selectBtn"
                                ...
                                期货转银行 <span class="caret" style="margin-left: 151px;"></span>
                            </button>
                    ...
                    <div class="form-group">
                        ...
                        <div class="spanCardNo">
                            <select type="text" style="display: inline;" v-model="cardName"     class="form-control form-text" id="firstname"
                                ...
                                <option v-for="name in cardInfo.data"    value="{{name['217']}},{{name['218']}},{{name['214']}},{{name['215']}},{{name['51']}},{{name['56']}},{{name['353']}}">{{name['216']}}</option>
                            </select>
                        </div>
                    </div>
                    <div class="form-group" v-show="billPassFlag">
                        <label for="bankPass" class="labelAll">资金密码：</label>
                        <div class="inputPosition">
                            <input type="password" class="form-control form-text" id="billPass" name="billPass"
                                   data-required="true" autocomplete="off"
                                   v-model="billPass" @focus="focusInfo"
                    ...
                    <div class="form-group" v-show="bankPassFlag">
                        ...
                        <div class="inputPosition">
                            <input type="password" class="form-control form-text" id="bankPass" name="bankPass"
                                   data-required="true" autocomplete="off"
                                   v-model="bankPass" @focus="focusInfo"
                    ...
                    <div class="form-group">
                        <label  class="labelAll">可取余额：</label>
                        <div class="inputPosition">
                            <input type="text" class="form-control form-text"
                                   id="availableBalance"
                                   value="{{vavibleMoney}}"
                                   v-model="availableBalance" @focus="focusInfo"
                                   data-required="true" readonly
                                   style="width: 140px;" autocomplete="off"
                                   data-regexp="^[0-9\.]+$"
                                   data-error-message="亲，请正确填写可取余额"
                    ...
                    <div class="form-group">
                        ...
                        <div class="inputPosition">
                            <input type="text" class="form-control form-text"   name="transMoney"
                                   @focus="billFocus"
                                   data-required="true"
                                   id="transMoney" autocomplete="off"
                                   v-model="transMoney"
                                   data-regexp="^[0-9\.]+$"
...
<script>
    ...
    import  {onlineGlobal,onlineGlobalOne,bankBlance,securityTurnBank,loginType,initMsgCode,outLoginType,checkSendMsgCode} from '../../util/config';
    ...
    export default{
        ...
        methods:{
            ...
            selectChange:function(){
                ...
                // 字段218期转银0:不要求;1:要求（废弃） 4需要资金密码 5需要银行密码 6 资金密码银行密码均要
                // 7银行密码 资金密码 均不要
                        ...
                        success:function(data){
                            if(data.retHead == 1){
                                ...
                                $.ajax({
                                    ...
                                    type:'post',
                                    timeout: 30000,
                                    data:JSON.stringify({func:securityTurnBank,type:loginType,account:account,token:token,
                                        data:[{
                                            56: _this.currency,
                                            214:_this.bankNo,
                                            215:_this.bankCode,
...
//                    alert(_this.bankNo)
                    ...
                    $.ajax({
                        ...
                        success:function(data){
                            if(getErrorMsg(data) == 1){
                                //取出银行余额
                                document.getElementById("availableBalance").value = data.data[0]['224']
//                                if(data.data[0]['210'] == '处理成功'|| data.data[0]['210'] =='1'){
//                                    if(data.data[0]['220'] !='')
//                                        document.getElementById("availableBalance").value = data.data[0]['220']
//                                    else
//                                        $('#monenyDetail').modal(monenyDetail);
//                                }else if(data.data[0]['210'] == '处理失败'){
//                                    _this.errorMessage = data.data[0]['211']
//                                }else if(data.data[0]['210'] == '处理中'){
//                                    _this.errorMessage = "查询超时，请在交易时段执行查询操作！"
//                                }
                                _this.bankBalance = true;
