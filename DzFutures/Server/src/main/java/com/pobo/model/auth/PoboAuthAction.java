...
public class PoboAuthAction {
	...
	private @Value("${auth.loginType}") static String loginType = "4";

	private @Value("${auth.orgnumber}") static String orgNumber = "1031";
...
//
//			String content;
//			content = httpService.doPost(loginUrl, map);
//			logger.debug("云认证登录，HTTP Post 返回，[{}]", content);
		} else {
...
//			map.put("arg3", userType);
//			map.put("arg4", DEV_ID);
//			map.put("arg7", OS);
//			String json = JsonUtil.obj2json(map);
//			map.clear();
//			map.put("json", json);
//			content = httpService.doPost(registerUrl, map);
//
...
//			map.put("arg4", DEV_ID);
//			map.put("arg7", OS);
//			protocol.put("protocol8", map);
//			String json = JsonUtil.obj2json(protocol);
//
//			httpService.setUrl(loginUrl);
//			content = httpService.send(json);
		} else {
			if ("0".equals(pack.GetString(1))) {
				map.put("retCode", pack.GetString(1));
				map.put("returnFlag", pack.GetString(1));
	...
	public Map<String, String> queryUserInfo(String poboNumber, String loginName,
		...
		logger.info("protocol_12: poboNumber=[{}], loginName=[{}], loginType=[{}], deviceId=[{}]",
				poboNumber, loginName, this.loginType, deviceId);
		poboPack pack = null;
		try {
			pack = j.protocol_12(poboNumber, loginName, this.loginType, deviceId, this.orgNumber);
		} catch (Exception e) {
			throw new DzException("云认证网络错误");
		}
	...
	public boolean checkToken(String account, String token) {
		poboPack pack = null;
		try {
			pack = j.protocol_6(null, account, DEV_ID, OS, null, token, null, null);
		} catch (Exception e) {
			e.printStackTrace();
		}
