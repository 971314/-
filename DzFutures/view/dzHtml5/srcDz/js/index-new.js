...
function showCon() {
  ...
  if (contract1) {
    ...
    if (contract.length == 2) {
      ...
      } else if (index0 >= 0 && index1 >= 0) {  //两个合约都在三个指数之列，找出三个指数中剩下的那个指数，加到contract头部
...
if (custom1) {
  ...
  if (!show.version || !show.checkedVer || show.version < iconInit.version) {  //没有版本号字段，或大版本号发生变化，直接覆盖本地配置文件的内容
    show = iconInit;
    pbEngine.writeLocalFile('H5LocalData1.json', JSON.stringify(show));
  } else if (show.version == iconInit.version && show.checkedVer < iconInit.checkedVer) {  //大版本号不变，小版本号发生变化，替换某些模块
    for (var i = 0; i < show['contents'].length; i++) {
      for (var j = 0; j < iconInit['contents'].length; j++) {
        if (show['contents'][i]['id'] == iconInit['contents'][j]['id']) {
          if (show['contents'][i]['title'] == iconInit['contents'][j]['title'] && show['contents'][i]['image1'] == iconInit['contents'][j]['image1'] && show['contents'][i]['image2'] == iconInit['contents'][j]['image2'] && show['contents'][i]['url'] == iconInit['contents'][j]['url']) {  //都没有发生变化，跳出循环
            break;
          } else {
...
keep = custom;
var length = custom.length, num;
if (length < 4) {
  num = 1;
  ReactDOM.render(<CustButtonO button={custom} />, document.getElementById('customized'));
} else if (length >= 4 && length < 8) {
  num = 1;
  ReactDOM.render(<CustButtonT button={custom} />, document.getElementById('customized'));
} else if (length >= 8 && length < 12) {
  num = 2;
  ReactDOM.render(<CustButtonTh button={custom} />, document.getElementById('customized'));  
} else if (length >= 12 && length < 16) {
  num = 2;
  ReactDOM.render(<CustButtonF button={custom} />, document.getElementById('customized'));  
} else if (length >= 16 && length < 20) {
  num = 3;
  ReactDOM.render(<CustButtonFi button={custom} />, document.getElementById('customized'));  
}
if (num >= 2) {
...
function readOrLogin(){
  if(window.pbEngine){
    ...
    if(data == undefined || data == 0){//如果等于0就是不存在就要去登录或则注册否则是游客
